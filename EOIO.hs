...
-}
 ...
 displayEOB (EOBoard fnds cols res) = do
...
-----------------------------------------------------------------
 ...
 scoreBoard (EOBoard fnds cols res) = "A LOSS: SCORE  " ++ (show (52- (foldr (+) 0 (map length res)) - (foldr (+) 0 (map length cols))))      
 ...
 displayEOGame b = do
  let (EOBoard fnds cols res) = b -- apparently can't do this with @
  displayEOB b -- display given board
  if ((length (filter (\n -> null n) cols)==8)&&(length (filter (\n -> null n) res)==8)) -- if cols & reserve empty its a win
     then return "A WIN"
     else do
